...
type Binder interface {
	Bind(bindDN, bindSimplePw string, conn net.Conn) (uint8, error)
}
type Searcher interface {
	Search(boundDN string, req SearchRequest, conn net.Conn) (ServerSearchResult, error)
}
type Adder interface {
	Add(boundDN string, req AddRequest, conn net.Conn) (uint8, error)
}
type Modifier interface {
	Modify(boundDN string, req ModifyRequest, conn net.Conn) (uint8, error)
}
type Deleter interface {
	Delete(boundDN, deleteDN string, conn net.Conn) (uint8, error)
}
type ModifyDNr interface {
	ModifyDN(boundDN string, req ModifyDNRequest, conn net.Conn) (uint8, error)
}
type Comparer interface {
	Compare(boundDN string, req CompareRequest, conn net.Conn) (uint8, error)
}
type Abandoner interface {
	Abandon(boundDN string, conn net.Conn) error
}
type Extender interface {
	Extended(boundDN string, req ExtendedRequest, conn net.Conn) (uint8, error)
}
type Unbinder interface {
	Unbind(boundDN string, conn net.Conn) (uint8, error)
}
...
// make 2 goroutines with channels: read from frontend, read from backend. They send data via channels
// make loop that recieves data from the channels, and invoke handler for each channel.
...
//Below here is the handler for received packet from frontend.
//It should write the packet to the backend
		...
		//log.Printf("DEBUG: handling operation: %s [%d]", ApplicationMap[uint8(req.Tag)], uint8(req.Tag))
...
//
func encodeLDAPResponse(messageID int64, responseType uint8, ldapResultCode uint8, message string) *ber.Packet {
...
func (h defaultHandler) Bind(bindDN, bindSimplePw string, conn net.Conn) (uint8, error) {
...
func (h defaultHandler) Add(boundDN string, req AddRequest, conn net.Conn) (uint8, error) {
	return LDAPResultInsufficientAccessRights, nil
}
func (h defaultHandler) Modify(boundDN string, req ModifyRequest, conn net.Conn) (uint8, error) {
	return LDAPResultInsufficientAccessRights, nil
}
func (h defaultHandler) Delete(boundDN, deleteDN string, conn net.Conn) (uint8, error) {
	return LDAPResultInsufficientAccessRights, nil
}
func (h defaultHandler) ModifyDN(boundDN string, req ModifyDNRequest, conn net.Conn) (uint8, error) {
	return LDAPResultInsufficientAccessRights, nil
}
func (h defaultHandler) Compare(boundDN string, req CompareRequest, conn net.Conn) (uint8, error) {
	return LDAPResultInsufficientAccessRights, nil
}
func (h defaultHandler) Abandon(boundDN string, conn net.Conn) error {
	return nil
}
func (h defaultHandler) Extended(boundDN string, req ExtendedRequest, conn net.Conn) (uint8, error) {
	return LDAPResultProtocolError, nil
}
func (h defaultHandler) Unbind(boundDN string, conn net.Conn) (uint8, error) {
